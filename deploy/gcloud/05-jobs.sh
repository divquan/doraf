...
for JOB in "${JOBS[@]}"; do
  ...
  if gcloud run jobs describe "dashchecker-${JOB}" --region="${REGION}" --project="${PROJECT_ID}" >/dev/null 2>&1; then
    gcloud run jobs update "dashchecker-${JOB}" \
      ...
      --update-secrets="DATABASE_URL=DATABASE_URL:latest,DASHCHECKER_CRYPTO_KEYS_JSON=DASHCHECKER_CRYPTO_KEYS_JSON:latest,PAYSTACK_SECRET_KEY=PAYSTACK_SECRET_KEY:latest" \
