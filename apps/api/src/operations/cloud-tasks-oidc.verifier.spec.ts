...
function createMockClient(payloadOrError: Record<string, unknown> | Error) {
  const verifyIdToken = jest.fn(async ({ idToken, audience }: { idToken: string; audience: string }) => {
...
describe('CloudTasksOidcVerifier', () => {
  ...
  it('throws 401 for missing token', async () => {
    const verifier = new CloudTasksOidcVerifier(createConfig(), new OAuth2Client() as any);
    await expect(verifier.verifyAuthorizationHeader(undefined)).rejects.toThrow('Missing bearer token');
  });

  it('throws 401 for malformed token', async () => {
    const verifier = new CloudTasksOidcVerifier(createConfig(), new OAuth2Client() as any);
    await expect(verifier.verifyAuthorizationHeader('BearerInvalid')).rejects.toThrow('Invalid bearer token');
    await expect(verifier.verifyAuthorizationHeader('Bearer ')).rejects.toThrow('Invalid bearer token');
  });
  ...
  it('throws 401 for expired token', async () => {
    ...
    const verifier = new CloudTasksOidcVerifier(createConfig(), client as any);
    await expect(verifier.verifyToken('expired')).rejects.toThrow('Token expired');
  });

  it('throws 403 for wrong audience', async () => {
    const wrongAudPayload = { ...validPayload, aud: 'https://other.example.com' };
    const { client } = createMockClient(wrongAudPayload);
    const verifier = new CloudTasksOidcVerifier(createConfig(), client as any);
    await expect(verifier.verifyToken('token')).rejects.toThrow('Invalid token audience');
  });

  it('throws 403 for wrong service account', async () => {
    const wrongEmailPayload = { ...validPayload, email: 'other@my-project.iam.gserviceaccount.com' };
    const { client } = createMockClient(wrongEmailPayload);
    const verifier = new CloudTasksOidcVerifier(createConfig(), client as any);
    await expect(verifier.verifyToken('token')).rejects.toThrow('Invalid token principal');
  });

  it('throws 401 for invalid signature', async () => {
    const { client } = createMockClient(new Error('Invalid signature'));
    const verifier = new CloudTasksOidcVerifier(createConfig(), client as any);
    await expect(verifier.verifyToken('bad')).rejects.toThrow('Invalid bearer token');
  });
  ...
  it('succeeds for valid token', async () => {
    ...
    expect(verifyIdToken).toHaveBeenCalledWith(expect.objectContaining({ audience: 'https://api.example.com/internal/tasks/outbox' }));
  });

  it('throws 403 when audience mismatch reported by library', async () => {
    const { client } = createMockClient(new Error('Wrong audience: expected https://api.example.com/internal/tasks/outbox'));
    const verifier = new CloudTasksOidcVerifier(createConfig(), client as any);
    await expect(verifier.verifyToken('token')).rejects.toThrow('Invalid token audience');
  });
  ...
  it('verifies via Authorization header', async () => {
    ...
    const verifier = new CloudTasksOidcVerifier(createConfig(), client as any);
    await expect(verifier.verifyAuthorizationHeader('Bearer valid-token')).resolves.toBeUndefined();
  });
});
