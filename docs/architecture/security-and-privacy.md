...
- phishing-resistant, user-verified passkeys are the MVP login method,
- authenticator-based MFA may be added later as an approved fallback,
...
Enrollment uses short-lived one-time tokens stored only as keyed fingerprints.
The first Administrator token is generated by a local bootstrap command that
refuses to run after any internal user exists. Later invitations require an
authenticated Administrator. See ADR-0011.
